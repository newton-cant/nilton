...
async function starts() {
  ...
  only: {
    ...
    daftarB: `🍒Oie, *${prefix}verify* para começar a usar o bot🍒`
...
*/
	...
	break
				...
				Pin.sendMessage(from, jes, image,{quoted : mek, caption : 'DONE'})
				break
...
method: 'get'
 ...
 Pin.sendMessage(from, yogi, image, {quoted: mek})
 break
 ...
 Pin.sendMessage(from, buffer, image, {caption: 'Jangan Cuma make doang Follow instagram.com/stardustlrlr', quoted: mek})
 break
...
case 'memeindo':
	...
	break
case 'jinmiran':
      data = fs.readFileSync('./data/jinmiran.js');
...
case 'mia':
      data = fs.readFileSync('./data/mia.js');
      jsonData = JSON.parse(data);
      randIndex = Math.floor(Math.random() * jsonData.length);
      randKey = jsonData[randIndex];
      buffer = await getBuffer(randKey.result)
...
case 'saycat':
      data = fs.readFileSync('./data/saycat.js');
      jsonData = JSON.parse(data);
      randIndex = Math.floor(Math.random() * jsonData.length);
      randKey = jsonData[randIndex];
      buffer = await getBuffer(randKey.result)
...
case 'porno':
      data = fs.readFileSync('./data/porno.js');
      jsonData = JSON.parse(data);
      randIndex = Math.floor(Math.random() * jsonData.length);
      randKey = jsonData[randIndex];
      buffer = await getBuffer(randKey.result)
...
quoted: mek, caption: teks
 ...
 case 'play':   
   play = body.slice(5)    
   anu = await fetchJson(`https://videfikri.com/api/ytplay/?query=${play}&apikey=`)
